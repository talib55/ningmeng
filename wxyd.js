...
微信阅读
需要青龙环境
入口，微信打开 -> https://kygj0209122415-1316151879.cos.ap-nanjing.myqcloud.com/index.html?upuid=10434947
...
class UserInfo {
  ...
  async getreadurl() {
    try {
      ...
      '1' == result.info.type &&
        1 !== this.dx &&
        (console.log(`文章获取成功 `), (this.b = 1), await $.wait(1000), await this.readfinish()),
        '3' == result.info.type &&
          1 !== this.dx &&
          (console.log(`已限制阅读   尝试过验证`),
          (this.x = result.info.key),
          (this.c = result.info.url.split('/s/')[1]),
          (this.b = 2),
          await $.wait(6000),
...
function popu(url, body = '', ck) {
  ...
  let urlObject = {
    ...
    headers: {
      ...
      Accept: '*/*',
      'User-Agent':
        'Mozilla/5.0 (Linux; Android 12; M2012K11AC Build/SKQ1.211006.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/86.0.4240.99 XWEB/4425 MMWEBSDK/20221206 Mobile Safari/537.36 MMWEBID/4883 MicroMessenger/8.0.32.2300(0x2800205D) WeChat/arm64 Weixin NetType/WIFI Language/zh_CN ABI/arm64',
...
`)
      ...
      setval(b, a) {
        return this.isSurge() || this.isLoon()
          ...
          : this.isNode()
          ? ((this.data = this.loaddata()), (this.data[a] = b), this.writedata(), !0)
          : (this.data && this.data[a]) || null
      }
      send(b, a, f = () => {}) {
        ...
        if (
          ('get' == b && a.headers
            ? (delete a.headers['Content-Type'], delete a.headers['Content-Length'])
            : a.body &&
              a.headers &&
              (a.headers['Content-Type'] || (a.headers['Content-Type'] = 'application/x-www-form-urlencoded')),
          this.isSurge() || this.isLoon())
        ) {
          this.isSurge() &&
            this.isNeedRewrite &&
            ((a.headers = a.headers || {}), Object.assign(a.headers, { 'X-Surge-Skip-Scripting': !1 }))
          ...
          (a.method = b.toUpperCase()),
            this.isNeedRewrite && ((a.opts = a.opts || {}), Object.assign(a.opts, { hints: !1 })),
            $task.fetch(a).then(
              (a) => {
      ...
      time(a) {
        ...
        for (let c in (/(y+)/.test(a) &&
          (a = a.replace(RegExp.$1, (new Date().getFullYear() + '').substr(4 - RegExp.$1.length))),
        b))
          new RegExp('(' + c + ')').test(a) &&
            (a = a.replace(RegExp.$1, 1 == RegExp.$1.length ? b[c] : ('00' + b[c]).substr(('' + b[c]).length)))
        return a
      }
      async showmsg() {
      ...
      msg(d = t, a = '', b = '', e) {
        let f = (a) => {
          if (!a) return a
          if ('string' == typeof a)
            return this.isLoon() ? a : this.isQuanX() ? { 'open-url': a } : this.isSurge() ? { url: a } : void 0
          if ('object' == typeof a) {
            if (this.isLoon()) {
